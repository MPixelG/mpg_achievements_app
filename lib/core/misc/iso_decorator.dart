...
/// [IsoDecorator] is an abstract class that encapsulates a particular visual
/// effect that should apply to drawing commands wrapped by this class.
///
/// The simplest way to apply a [IsoDecorator] to a component is to override its
...
class IsoDecorator {
  ...
  void applyChain(void Function(Canvas, Canvas?) draw, Canvas canvas, Canvas? depthCanvas) {
    apply(
      _next == null
          ? draw
          : (nextCanvas, nextCanvasDepth) => _next!.applyChain(draw, nextCanvas, nextCanvasDepth),
      canvas,
      depthCanvas,
    );
  }
  ...
  void addLast(IsoDecorator? decorator) {
    if (decorator != null) {
      if (_next == null) {
        _next = decorator;
      } else {
  ...
  void replaceLast(IsoDecorator? decorator) {
    if (decorator == null) {
      removeLast();
