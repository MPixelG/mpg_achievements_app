...
class TileInstance {
  ...
  final int zIndex;
  TileInstance(this.sprite, this.position, this.zIndex);
}

class IsometricTiledLevel extends TiledComponent {
  final List<TileInstance> _tiles = [];
  ...
  Future<void> _addTileForGid(
    ...
    _tiles.add(TileInstance(sprite, worldPos, tileZ));
  }
  ...
  void render(Canvas canvas) {
    canvas.drawRect(Rect.fromPoints(Offset.zero, scaledSize.toOffset()), paint);

    for (final tile in _tiles) {

      canvas.drawRect(Rect.fromPoints(tile.position.toOffset(), tile.position.toOffset() + Offset(4, 4)), paint);
