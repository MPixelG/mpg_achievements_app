...
class AdvancedCamera extends CameraComponent {
  ...
  // easing helpers
  double linear(double t, {double startVal = 0, double endVal = 1}) => startVal + (endVal - startVal) * t;
  double easeIn(double t, {double startVal = 0, double endVal = 1}) => startVal + (endVal - startVal) * (t * t);
  double easeOut(double t, {double startVal = 0, double endVal = 1}) => startVal + (endVal - startVal) * (1 - (1 - t) * (1 - t));
  double easeInOut(double t, {double startVal = 0, double endVal = 1}) =>
      startVal + (endVal - startVal) * ((t < 0.5) ? (2 * t * t) : (1 - 2 * (1 - t) * (1 - t)));
  ...
  void update(double dt) {
    ...
    if (followPlayer) { //if the camera is in follow mode we check if we need to reposition the camera to the player
      if (player != null && viewfinder.position.distanceTo(player!.position) > followAccuracy) {
        // use a short smooth move instead of instant set to avoid small jumps
        moveTo(player!.absoluteCenter, time: 250, animationStyle: AnimationStyle.easeOut);
      }
    }
    ...
    if(shakingTime > 0) { //if theres an ongoing shaking animation
      ...
      double power = switch (shakingAnimation){
        AnimationStyle.easeIn => easeIn(1 - ratio, startVal: shakingAmount, endVal: 0), //basically the same as the moving of the camera
        AnimationStyle.easeOut => easeOut(1 - ratio, startVal: shakingAmount, endVal: 0),
        AnimationStyle.linear => linear(1 - ratio, startVal: shakingAmount, endVal: 0),
        AnimationStyle.easeInOut => easeInOut(1 - ratio, startVal: shakingAmount, endVal: 0),
      };
