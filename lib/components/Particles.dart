...
ParticleSystemComponent generateConfetti(Vector2 pos){
  ...
  Particle particle = Particle.generate(
    count: 5000, //low-end/older devices like mine have problems with this amount of particles
    generator: (i) {
      Vector2 position = Vector2.zero();
      Vector2 velocity = randomVector2(); //generates random velocity
      ...
      return ComputedParticle(
        renderer: (canvas, _) {
          ...
          velocity *= 0.94 + (size / 200);// The larger the confetti, the slower it will move (based on size)
          position += velocity; // Update the position of the particle based on its velocity
          angle += (size / 200).toInt();  // Change the angle of the particle slightly based on its size (larger particles rotate more)
          canvas.rotate(radians(angle.toDouble())); // Rotate the canvas to apply the angle to the particle before drawing it
