...
abstract class Goal extends Component {
  ...
  void update(double dt) {
    if (!done) {
      //as long as the goal isnt done, we perform a step.
      step();
    } else {
...
mixin GoalIncludesBasicMovement on PositionComponent, BasicMovement {
  ...
  void recalculatePath(Vector2 endPos) {
    ...
    stepIndex = 0;
    timeSinceLastPosChange = 0;
  }

  PathStep? currentStep;
  int stepIndex = 0;

  double timeSinceLastPosChange = 0;
  ...
  void updateGoal(double dt) {
    ...
    Vector2 currentStepPosCenter =
        currentStep!.node.poiNode.position * 32 + Vector2.all(16);
    ...
    if (absoluteCenter.distanceTo(currentStepPosCenter) < 15 &&
    ...
    switch (currentStep?.action) {
      ...
      case PathfindingAction.climbUp:
        {
          climb(currentStepPosCenter);
          adjustHorizontalMovementForClimbing();
          break;
        }
    ...
    if(((lastPosition).clone()..floor()) == ((position).clone()..floor())) timeSinceLastPosChange += dt;
    lastPosition = position.clone();

    if(timeSinceLastPosChange > 4 && currentStep != null) {recalculatePath(endPos); timeSinceLastPosChange = 0; position.y -= 10; }
    else {
      timeSinceLastPosChange = 0;
    }
  }
  ...
  PathStep getNearestStep(Vector2 position, List<PathStep> path) {
    return path.reduce((value, element) {
      ...
      PathStep nearestTarget =
          ((value.node.poiNode.position * 32).distanceTo(position) <
              (element.node.poiNode.position * 32).distanceTo(position))
          ? value
          : element;

      if (hasClearPath(
        nearestTarget.node.poiNode.position + Vector2.all(0.5),
        position / 32,
      )) {
